package com.sickkids.caliper.service
{
	...
	import com.sickkids.caliper.vo.AllApplicationBean;
	...
	public class HomeRemoteObjectService extends Actor implements IHomeRemoteObjectService
	{
		private var _remoteObject:RemoteObject;
		[Inject] public var homeModel:HomeModel;
		
		public function HomeRemoteObjectService(dest:String="carmProxy", endpoint:String="{FlexGlobals.topLevelApplication._host}/carm/messagebroker/amf")
		{
			trace("INFO: HomeRemoteObjectService() is called in HomeRemoteObjectService.as, endpoint="+endpoint);
		...
		{		
			...
			trace("INFO: onOperationResult() is called in HomeRemoteObjectService.as, op="+op+", e.result="+e.result);
			if(e.token.message.destination=="carmProxy")
			{	
				switch(op)
				{
					case "getAppList": 
							//dispatch(new HomeRMIEvent(HomeRMIEvent.GET_APP_LIST, e.result as Boolean));
							putApplications(e.result as ArrayCollection);
							break;
					/*
					case "getTestString":
						break;
