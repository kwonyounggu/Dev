 ...
 * 1) getQueryResults. Connects to a database, executes a query, retrieves all
 * the rows as arrays of strings, and puts them inside a DBResults object. Also
 * places the database product name, database version, and the names of all the
 * columns into the DBResults object. This has two versions: one that makes a
 * new connection and another that uses an existing connection.
 * <P>
 * 2) createTable. Given a table name, a string denoting the column formats, and
 * an array of strings denoting the row values, this method connects to a
 * database, removes any existing versions of the designated table, issues a
 * CREATE TABLE command with the designated format, then sends a series of
 * INSERT INTO commands for each of the rows. Again, there are two versions: one
 * that makes a new connection and another that uses an existing connection.
 * <P>
 * 3) printTable. Given a table name, this connects to the specified database,
...
public class DatabaseUtilities
{
	...
	/**
	 * Connect to database, execute specified query, and accumulate results into
	 * DBRresults object. If the database connection is left open (use the close
	 * argument to specify), you can retrieve the connection with
	...
	/**
	 * Retrieves results as in previous method but uses an existing connection
	 * instead of opening a new one.
	 */

	public static DBResults getQueryResults(Connection connection, String query, boolean close)
	{
		try
		{
			...
			DBResults dbResults = new DBResults(connection, productName, productVersion,columnCount, columnNames);
	...
	public static Connection createTable(String driver, String url,
			...
			String tableFormat, String[] tableRows, boolean close)
	{
		try
		{
			Class.forName(driver);
			Connection connection = DriverManager.getConnection(url, username,
					password);
			return (createTable(connection, username, password, tableName,
					tableFormat, tableRows, close));
		}
		catch (ClassNotFoundException cnfe)
		{
			System.err.println("Error loading driver: " + cnfe);
	...
	/**
	 * Prints out all entries in a table. Each entry will be printed in a column
	 * that is entryWidth characters wide, so be sure to provide a value at
	 * least as big as the widest result.
	 */
	...
	public static void printTableData(String tableName, DBResults results,
			int entryWidth, boolean printMetaData)
	{
