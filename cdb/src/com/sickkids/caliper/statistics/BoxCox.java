...
public class BoxCox
{
	...
	public BoxCox(Double[] d)
	{
		...
		Double[] ESD = StatisticalTests.getEandSD(transformAll(bestP, bestA,
				originalData));
	...
	public Double[] getSTDTransformedData(Double[] data)
	{
		...
		{
			transformed[i] = new Double(
					(transformed[i].doubleValue() - transformedMean)
							/ transformedSD);
		}
		return transformed;
	}
	...
	private void getParamWhichMaximizeLogLikelihood()
	{
		...
		{
			if (minA > originalData[i].doubleValue()) minA = originalData[i]
					.doubleValue();
		}
		minA = -minA + TINY;
		...
		try
		{
			if (logLikelihood(1 + (maxP - minP) / (double) partitionP, minA) > logLikelihood(
					1, minA)) direction = 1;
			else direction = -1;
		}
		catch (Exception e1)
		{
			System.err.println(" è¦È[¢I");
			System.exit(-1);
		}
		for (int j = 0; j <= partitionA; ++j)
		{
			...
			{
				double thisP = 1 + direction * i * (maxP - minP)
						/ (double) partitionP;
				...
				try
				{
					...
					// thisLL=
					// -Math.abs(StatisticalTests.getSampleSkewness(transformAll(thisP,thisA,originalData)));
				}
				catch (Exception e)
				{// if thisLL==Infinity
					continue;
				...
				{
					// break;//LogLikelihood must? be increasing along the
					// direction.
				}
				// System.out.print(thisLL+",");break;
			}
			// System.out.println("},{");
			// if(bestP<=minP || bestP>=maxP){//if(! LogLikelihood maximized in
			// [minP,maxP] )
			// continue;
			// }else return;
			// bestA = minA;
	...
	private double logLikelihood(double p, double a) throws Exception
	{
		...
		double value = -originalData.length / 2.0
				* Math.log(2 * Math.PI * Math.pow(ESD[1].doubleValue(), 2))
				- originalData.length * (originalData.length - 1) / 2.0;
		for (int i = 0; i < originalData.length; ++i)
		{
	...
	private Double transform(double p, double a, Double d)
	{
		...
		if (Double.isNaN(y) || Double.isInfinite(y) || y <= -Double.MAX_VALUE
				|| Double.MAX_VALUE <= y)
		{
			// throw new
			// SCMDException("BoxCox.transform() p="+p+" y="+y+" x-a="+(x-a));
			System.err.println("BoxCox.transform() p=" + p + " y=" + y
					+ " x-a=" + (x - a));
			System.exit(-1);
		}
		return new Double(y);
	}
	public double reverseTransform(double d)
	{
		return reverseTransform(bestP, bestA, Double.valueOf(d)).doubleValue();
	}
	private Double reverseTransform(double p, double a, Double d)
	{
		...
		if (Double.isInfinite(x) || x <= -Double.MAX_VALUE
				|| Double.MAX_VALUE <= x || 1.7976931348623157E308 == x)
		{
			// throw new
			// SCMDException("BoxCox.transform() p="+p+" y="+y+" x-a="+(x-a));
			System.out.println("BoxCox.reverseTransform() p=" + p + " y=" + y
					+ " x-a=" + (x - a));
			System.exit(-1);
		}
		if (Double.isNaN(x))
	...
	public static final double[] pValueList = { 0.0000118, 0.000118, 0.01,
			0.005, 0.001, 0.0005, 0.0002, 0.0001, 0.00005, 0.00001, 0.000005,
			0.000001, 1E-10 };

	// public static final double[]
	// pValueList={0.01,0.001,0.0001,0.00001,0.000001};
	...
	{
		String t = "\t";
		pw.print("LogLikelihood" + t + "p" + t + "a" + t + "transformedMean"
				+ t + "transformedSD" + t);
	...
	{
		String t = "\t";
		pw.print(maxLogLikelihood + t + bestP + t + bestA + t + transformedMean
				+ t + transformedSD + t);

		for (int i = 0; i < pValueList.length; ++i)
		{
			pw.print(getOneSidedCriticalValueForP(
					pValueList[pValueList.length - 1 - i]).doubleValue()
					+ t);
		}
		for (int i = 0; i < pValueList.length; ++i)
		{
			pw.print(getOneSidedCriticalValueForP(1 - pValueList[i])
					.doubleValue() + t);
		}
	}

	// public static final double[] sigmaValueList={0.5,1,1.5,2,2.5,3};
	public static final double[] sigmaValueList = { 2.3263, 3.0902, 3.7190,
			4.2649, 4.7534, 5.1993, 5.6120, 5.9978 };

	public Double getOneSidedCriticalValueForMeanPlusDSigma(double dsigma)
	{
		Double[] transformed = transformAll(bestP, bestA, originalData);
		return reverseTransform(bestP, bestA, new Double(transformedMean
				+ dsigma * transformedSD));
	}
	...
	public Double getOneSidedCriticalValueForMaxPlusSigma(double dsigma)
	{
		...
		return reverseTransform(bestP, bestA, new Double(maxValue + dsigma
				* transformedSD));
	}
	...
	public Double getOneSidedCriticalValueForMinMinusSigma(double dsigma)
	{
		...
		return reverseTransform(bestP, bestA, new Double(minValue - dsigma
				* transformedSD));
	}
	...
	public void printSigma(PrintWriter pw)
	{
		...
		for (int i = 0; i < sigmaValueList.length; ++i)
		{
			pw.print(getOneSidedCriticalValueForMeanPlusDSigma(
					-sigmaValueList[sigmaValueList.length - 1 - i])
					.doubleValue() + t);
