...
public class StatisticalTests
{
	...
	static public Double[] getStandardizedData(Double[] data)
	{
		...
		for (int i = 0; i < data.length; ++i)
		{
			standard[i] = new Double(
					(data[i].doubleValue() - ESD[0].doubleValue())
							/ ESD[1].doubleValue());
		}
		return standard;
	}

	static public double getSampleSkewness(Double[] data) throws Exception
	{
		Double[] ESD = getEandSD(data);
		double sampleMean = ESD[0].doubleValue();
		if (ESD[1].doubleValue() == 0) { return 0.0; }
		double a = 0, b = 0;
		for (int i = 0; i < data.length; ++i)
		{
			double d = data[i].doubleValue();
		...
		if (Double.isNaN(result)) { throw new Exception(
				"Error. StatisticalTests.getSampleSkewness() returned NaN. data.length="
						+ data.length + " a=" + a + " b=" + b + " sampleMean="
						+ sampleMean);
		/***
		 * System.err.println(
		 * "Error. StatisticalTests.getSampleSkewness() returned NaN. data.length="
		 * +data.length+" a="+a+" b="+b+" sampleMean="+sampleMean); for(int
		 * i=0;i<data.length;++i){ System.err.println(data[i].doubleValue()); }
		 * System.exit(-1);
		 ***/
		}
		return result;
	...
	static public double getSampleKurtosis(Double[] data) throws Exception
	{
		Double[] ESD = getEandSD(data);
		double sampleMean = ESD[0].doubleValue();
		if (ESD[1].doubleValue() == 0) { return 0.0; }
		...
		if (Double.isNaN(result)) { throw new Exception(
				"Error. StatisticalTests.getSampleKurtosis() returned NaN. data.length="
						+ data.length + " a=" + a + " b=" + b);
		/***
		 * System.err.println(
		 * "Error. StatisticalTests.getSampleKurtosis() returned NaN. data.length="
		 * +data.length+" a="+a+" b="+b); for(int i=0;i<data.length;++i){
		 * System.err.println(data[i].doubleValue()); } System.exit(-1);
		 ***/
		}
		return result;
	}
	...
	static public double HastingsApproximationForNormalDF(double mean,
			double sd, double x)
	{
		return HastingsApproximationForStandardNormalDF((x - mean) / sd);
	}

	static public double HastingsApproximationForStandardNormalDF_Reverse(
			double p)
	{
		if (p <= 0 || 1 <= p)
		{
			System.err
					.println("Error in StatisticalTests.HastingsApproximationForReverseStandardNormalDF("
							+ p + ") assert(0<x<1)");
			System.exit(-1);
		}
		int sign = 1;
		...
		double returnValue = z
				- (coeff_a[0] + coeff_a[1] * z + coeff_a[2] * z * z)
				/ (1 + coeff_b[0] * z + coeff_b[1] * z * z + coeff_b[2] * z * z
						* z);
		return -sign * returnValue;
	}

	static public double HastingsApproximationForNormalDF_Reverse(double mean,
			double sd, double p)
	{
	...
	static final double[] ChiSquareTableUpper0_1 = { -1, 2.706, 4.605, 6.251,
			7.779, 9.236, 10.645, 12.017, 13.362, 14.684, 15.987, 17.275,
			18.549, 19.812, 21.064, 22.307, 23.542, 24.769, 25.989, 27.204,
			28.412 };
	static final double[] ChiSquareTableUpper0_01 = { -1, 6.635, 9.210, 11.345,
			13.277, 15.086, 16.812, 18.475, 20.090, 21.666, 23.209, 24.725,
			26.217, 27.688, 29.141, 30.578, 32.000, 33.409, 34.805, 36.191,
			37.565 };
	static final double[] ChiSquareTableUpper0_001 = { -1, 10.828, 13.816,
			16.266, 18.467, 20.515, 22.458, 24.322, 26.125, 27.877, 29.588,
			31.264, 32.910, 34.528, 36.123, 37.697, 39.252, 40.790, 42.312,
			43.820, 45.315 };

	static public double upperCriticalValueOfChiSquareDistribution(
			int degreeOfFreedom, double prob)
	{
		if (degreeOfFreedom < 1 || 20 < degreeOfFreedom)
		{
			System.err
					.println("Error in Statistics.upperCriticalValueOfChiSquareDistribution(): no critical value table for degree of freedom ="
							+ degreeOfFreedom);
			System.exit(-1);
		}
		...
		{
			System.err
					.println("Error in Statistics.upperCriticalValueOfChiSquareDistribution(): no critical value table for prob="
							+ prob);
			System.exit(-1);
		}
		return returnValue;
	...
	public static void main(String[] args)
	{
		...
		{
			// double
			// d=HastingsApproximationForStandardNormalDF_Reverse(i/10.0);
			// System.out.println(i/10.0+"\t"+d);
			double d = HastingsApproximationForStandardNormalDF_Reverse(Math
					.pow(10, -i));
